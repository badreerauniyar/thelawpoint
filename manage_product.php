	...
	if($msg==''){
		if(isset($_GET['id']) && $_GET['id']!=''){
			if($_FILES['image']['name']!=''){
				...
				$update_sql="update product set categories_id='$categories_id',name='$name',date='$date',author='$author',image='$image' where id='$id'";
			}else{
				$update_sql="update product set categories_id='$categories_id',name='$name',date='$date',author='$author', where id='$id'";
			}
			mysqli_query($con,$update_sql);
		}else{
			$image=rand(111111111,999999999).'_'.$_FILES['image']['name'];
			move_uploaded_file($_FILES['image']['tmp_name'],PRODUCT_IMAGE_SERVER_PATH.$image);
			mysqli_query($con,"insert into product(categories_id,name,date,author,status,image) values('$categories_id','$name','$date','$author',1,'$image')");
		}
		header('location:product.php');
		die();
	}
                        ...
                        <form method="post" enctype="multipart/form-data">
							<div class="card-body card-block">
							   <div class="form-group">
								...
								<div class="form-group">
									<label for="categories" class=" form-control-label">News headline</label>
									<input type="text" name="name" placeholder="Enter news headline" class="form-control" required value="<?php echo $name?>">
								</div>
